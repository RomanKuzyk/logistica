...
### 2026-04-11
- На BAF-рівні зафіксовано та частково виправлено проблему author attribution для mobile receive/unpacking/photo flows.
- Підтверджено крос-системну причину:
  - `api-nodejs` ходить у 1С під технічним користувачем `web`;
  - BAF історично проставляла `Автор` із `ПараметрыСеанса.ТекущийПользователь`;
  - legacy mobile app при цьому не передає явний `UserIdRef` / `condigoId` у проблемних RPC-функціях.
- Прийняте сумісне рішення без перевипуску mobile app:
  - BAF для mobile-linked receive/unpacking/file/photo flows тепер намагається визначати автора через бізнес-виконавця (`Исполнитель`) замовлення / викупу;
  - fallback на `web` лишається тільки коли визначити користувача неможливо.
- Деталі й межі рішення:
  - `baf/docs/mobile-author-attribution.md`
